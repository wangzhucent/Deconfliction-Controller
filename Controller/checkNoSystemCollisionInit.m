function outputSys=checkNoSystemCollisionInit(i,Sys)
% int i
% Sys.Agents
% Sys.ActivatedCenter
% disp('!!')
    ...
    for j=1:length(Sys.Agents)
        if j==i
            continue;
        end % preventing checking the same agents
        agent2=Sys.Agents(j);
        ...
        while (l1<6*40 && l2<6*40) && i1<size(agent1.path,1) && i2<size(agent2.path,1)
            if norm(path1(i1,1:2)-path2(i2,1:2))<15            
                ...
                while leave_l1-l1<2*rho && leave_i1<size(agent1.path,1)
                    leave_l1=leave_l1+norm(path1(leave_i1+1,1:2)-path1(leave_i1,1:2));
                    leave_i1=leave_i1+1;
                end

                while leave_l2-l2<2*rho && leave_i2<size(agent2.path,1)
                ...
                output1.agent_num=j;
                output2.agent_num=i; %store the agent code who it will have collision with 
                
                output1.isActivated=0;
                output2.isActivated=0; % a flag for activation
                
                outputSys=Sys;
                outputSys.Agents(i).confliction=output1;
                outputSys.Agents(j).confliction=output2;
                outputSys.Agents(i).speedmode=output1.speedmode;
                outputSys.Agents(j).speedmode=output2.speedmode;
                return;
            end
            if l1<=l2
                i1=i1+1;
