function output=controlSystem3(Sys,rho,eps,dt)

    Agents=Sys.Agents;
    ActivatedCenter=Sys.ActivatedCenter;
    m=size(Agents,1);
    output=Agents;
for i=1:m
    curAgent=Agents(i);
    if curAgent.confliction.agent_num==0    %have a temperory confliction center
        
        
        
    else    %not have a temporary confliction center
